...
raw_cor <- jsonlite::read_json(sprintf("%s/cor/raw.json", result_dir))
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
countries_cities <- read.xlsx(sprintf("%s/cor/abs_countries_cities.xlsx", result_dir))
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
genes <- read.xlsx(sprintf("%s/cor/abs_genes.xlsx", result_dir))
